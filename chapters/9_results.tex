...
The goal of this experiment is to measure the alignment error of the long axis of the surgical tool with the fulcrum point. In other words, this error tracks whether the robot pose satisfies the RCM constraint, 
in which the axis of the tool must always pass through the fulcrum point. To calculate this error, the line of the long axis of the surgical tool must be first defined. To define this line, 2 points are needed. Both of these 
...
\end{bmatrix}
\end{equation}
The covariance matrix and its coefficients can be calculated using equations \ref{eq:cov-matrix} and \ref{eq:cov-matrix-coeff}, using the mean values of the estimations of $x,y,z,ψ,θ,φ$. To simplify the experiment, 
...
Since both the origin of the fulcrum reference frame and the line of the long axis of the tool are known then the alignment error of this experiment can be calculated as the distance of the line $l$ from the point $O_F$
\begin{equation}
e_{rcm} = d(l, O_F)
\end{equation}
...
\caption{ROS nodes and topics used for the robot-planner6 experiment.}
\end{figure}
\end{center}
