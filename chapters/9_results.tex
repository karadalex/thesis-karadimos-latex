...
In this first simulation we are testing some simple trajectories with the surgical tool already attached to the robot arm's end effector. The path is designed using the appropriate coordinates and orientations so that the robot begins from the home position, then visits the table with the surgical 
tools and then visits the other table on top of which the mounting dock is placed. Upon arrival at the mounting dock, the robot inserts the tool inside a hole (we consider these holes to be a simplistic alternative to the trocars used in real operations), then executes a simple pivot motion, while the tool is still inserted and then the tool gets ejected from the mounting dock's hole.
The aim of this simulation is to test the overall behaviour of the robot inside the work space, before implementing more complex path planning algorithms. In the sequel all simulations start from the home position, while the used parameters are: tolerances: 0.000005, max planning time: 5 seconds, replanning: true, max planning attempts: 6. Furthermore, the `execution status' is 1 for success and 0 for failure.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner1.png}
...
\hline
Planner 1           & \multicolumn{4}{c}{\textbf{RRTConnect}}                                                                                                 \vline \\
\hline
...
\hline
\caption{Time results for Robot Planner (RP) 1 using the RRTConnect path planner algorithm. Planning time is the sum of solution time and path simplification time. %Execution status is 1 for success and 0 for failure.
}
...
\begin{longtable}{|p{2cm}|c|p{2cm}|p{2cm}|p{2cm}|}
\hline
Planner 1           & \multicolumn{4}{c}{\textbf{RRT*}}                                                                                                 \vline \\
\hline
                          & \multicolumn{4}{c}{\textbf{Fake tool picking trajectory}}                     \vline \\
\hline
Simulation                & Approach planning time & Execution status & Move away planning time & Execution status  \\
...
\hline
\caption{Time results for robot planner 1 using the RRT* path planner algorithm. Planning time is the sum of solution time and path simplification time. %Execution status is 1 for success and 0 for failure. %Parameters: tolerances: 0.000005, max planning time: 5 seconds, replanning: true, max planning attempts: 6.
}
\label{robot-planner1-rrtstar-data}
\end{longtable}


\section{Robot Planner 2: Simulation layout and reachability studies}
\label{section:robot-planner2}

In this simulation, we plan a path such that the robot arm will visit all holes of the mounting dock and will try the insertion movement of the surgical tool.
This study is useful, because it shows whether all holes of the mounting dock are \textbf{reachable} (inside the robot's work space) and if so, how 
\textbf{dexterous} the robot will be in pivoting around each hole, i.e. how free the robot arm is to execute pivot motions.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner2a}
\caption{Simulation 2a:}
\label{Simulation-robot-planner2a}
\end{figure}
\end{center}
...
\begin{longtable}{|p{2cm}|c|p{2cm}|p{2cm}|p{2cm}|}
\hline
Planner 2a           & \multicolumn{4}{c}{\textbf{RRTConnect}}                                                                                                 \vline \\
\hline
                          & \multicolumn{4}{c}{\textbf{Insertion \& Pivot trajectories}}                     \vline \\
\hline
...
\caption{Time results for RP 2a using the RRTConnect path planner algorithm. Planning time is the sum of solution time and path simplification time. %Execution status is 1 for success and 0 for failure. %Parameters: tolerances: 0.0005, max planning time: 5 seconds, replanning: true, max planning attempts: 6.
}
\label{robot-planner2a-rrtconnect-data}
\end{longtable}

\normalsize
To overcome the reachability issue shown in Figure \ref{Simulation-robot-planner2a}, the algorithm was repeated, but this time using a different simulation layout 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner2b}
\caption{Simulation 2b: Running robot planner 2b with a different table layout so that all target poses have improved reachability. Note that all trajectories shown in the screenshots are collision-free, 
...
\label{Simulation-robot-planner2b}
\end{figure}
\end{center}

\normalsize
Observing the screenshots at Figure \ref{Simulation-robot-planner2b}, it is clear that the robot must always be in an elbow-up configuration in order to avoid collisions with the mounting dock. This constraint can be described either using the relative distance of the third link with the base or the relative angles of the third link's axis with respect to the axis of the base, as described with more detail in 
...
link, instead of the end-effector. To avoid these additional, computationally expensive calculations and simplify the simulations, instead of mathematically describing the constraint, 
...
configuration for most of the path, because otherwise it would make a big jump in the joint state space which is not allowed from the parameters used in the simulations.

% Robot Planner 2b with RRTConnect
\begin{longtable}{|p{2cm}|c|p{2cm}|p{2cm}|p{2cm}|}
\hline
Planner 2b           & \multicolumn{4}{c}{\textbf{RRTConnect}}                                                                                                 \vline \\
\hline
                          & \multicolumn{4}{c}{\textbf{Insertion \& Pivot trajectories}}                     \vline \\
\hline
Simulation                & Trocar1 insertion time & Execution status & Trocar1 retraction time & Execution status  \\
...
\hline
\caption{Time results for RP 2b using the RRTConnect path planner algorithm and a different table layout from RP 2a. Planning time is the sum of solution time and path simplification time. %Execution status is 1 for success and 0 for failure. %Parameters: tolerances: 0.0005, max planning time: 5 seconds, replanning: true, max planning attempts: 6.
}
\label{robot-planner2b-rrtconnect-data}
\end{longtable}


Due to the probabilistic nature of the motion planner (in these simulations the OMPL library is used with the RRTConnect path planning algorithm), the solutions 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner2b/singularity_failure.png}
\caption{Simulation 2b: Singularity failure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/kinematic_state_topic_graph.png}
...
The goal of this third simulation is to design and test only some pivot trajectories. The pivot motions follow the equations described in 
\ref{section:pivot-motions}. The trajectories that were designed and tested in this group of simulations are the following:
\begin{itemize}
\item Line segment
\item Circle
\item Cubic spline
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3b_line_seg.png}
\caption{Simulation 3b: Create the line segment trajectory inside the surgical site (below the green mounting dock) and transform it via the fulcrum transformation to a trajectory for the robot's TCP.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/robot_planner3b_error.png}
\caption{Simulation 3b: RCM error diagram from home position to line and reverse-line segment trajectories.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/robot_planner3b-error_distributions.png}
\caption{Simulation 3b: RCM error distributions, measurements from 10 iterations of the same simulation. From left to right: distribution of all measurements, distribution of measurements while the robot was pivoting, 
...
\hline
\end{tabular}
\caption{Accuracy and repeatability of the line segment simulation, calculated using measurements from 10 iterations of the same simulation}
\end{table}

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.35\textwidth]{images/robot_planner3/robot_planner3b_manip1.png}
\includegraphics[width=0.35\textwidth]{images/robot_planner3/robot_planner3b_manip2.png}
\caption{Simulation 3b: Manipulability plots of the whole trajectory the robot executed during 2 iterations of the same simulation}
\label{robot-planner3b-line-seg-manipulability-plots}
\end{figure}
\end{center}

Note that although this type of trajectory is trivial in RP, this specific line segment trajectory should not be confused with ROS MoveIt line segment trajectories. The line-segment studied in this simulation is linear when defined in the surgical taskpsace, but it is highly non-linear when transformed via the fulcrum transformation and it has a curved shape. The only exceptions to this transformation is for line segments that lie on the direction defined by the radius unit vector $\mathbf{\hat{r}}$ of the spherical coordinate system of the Fulcrum reference frame, or equivalently the points $A,B$ defining the line-segment and the origin $O$ of the reference frame, must be collinear. These exceptions can also be planned by the MoveIt framework and are also known as and referenced throughout this thesis, as insertion movements. Let $\overrightarrow{AB}$ be the vector representing the line segment, then for the points $A, B$ the following must hold, in order for the line segment to be invariant under the fulcrum transformation in terms of shape (i.e. the line to remain a line).
\begin{equation}
\overrightarrow{AB} = \overrightarrow{OB} - \overrightarrow{OA},\mbox{and }
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3b_line_seg_invariant.png}
\caption{Simulation 3b: Line segment trajectory that is invariant under the fulcrum transformation, in terms of shape (the line remains a line).}
...
\hline
Planner 3b          & \multicolumn{4}{c}{Approach and \textbf{line segment pivot} trajectories with \textbf{RRTConnect}}                                                                                                 \vline \\
\hline
...
\hline
Simulation                & Approach fulcrum 2 path planning time (sec) & Execution status & Insertion path planning time (sec) & Execution status  \\
\hline
...
\hline
\caption{Time results for RP 1 using the RRTConnect path planner algorithm. Planning time is the sum of solution time and path simplification time. %Execution status is 1 for success and 0 for failure. %Parameters: tolerances: 0.000005, max planning time: 5 seconds, replanning: true, max planning attempts: 6.
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3a_circle.png}
\caption{Simulation 3a: Create circular trajectory inside the surgical site (below the green mounting dock) and transform it via the fulcrum transformation to a trajectory for the robot's TCP.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3a_circle_details.png}
\caption{Simulation 3a: A more detailed view of the circular trajectories. (a) the original circular trajectory designed inside the surgical taskspace, (b) the transformed trajectory that the base of the surgical tool will
 follow, (c) the actual transformed trajectory that the robot's end-effector will follow}
\label{robot-planner3a-circle-details}
\end{figure}
\end{center}

Note that in this simulation (see screenshot \ref{robot-planner3a-circle}), the trivial case is examined where the circle is parallel to the $xy$ plane of the fulcrum's 
...
The goal of this simulation is to generate a cubic spline trajectory inside the surgical
taskspace which will then be transformed via the fulcrum transformation to a trajectory that the robotic arm can execute. Cubic spline equations are presented in more detail at \ref{section:cubic-spline} and the generated trajectories of this simulation can be seen in Figure~ \ref{robot-planner3c-cubic-spline}.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3c_cubic_spline.png}
\caption{Simulation 3c: Create the cubic spline trajectory inside the surgical site (below the green mounting dock) and transform it via the fulcrum transformation to a trajectory for the robot's TCP.}
...
The goal of this simulation is to generate a B-Spline trajectory inside the surgical
taskspace which will then be transformed via the fulcrum transformation to a trajectory that the robotic arm can execute. B-spline equations are presented in more detail at \ref{section:b-spline} and the generated trajectories 
of this simulation can be seen in figure \ref{robot-planner3d-bezier-spline}.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3d_bezier_spline.png}
\caption{Simulation 3d: Create the B-spline trajectory inside the surgical site (below the green mounting dock) and transform it via the fulcrum transformation to a trajectory for the robot's TCP.}
...
The goal of this simulation is to generate a trajectory in joint space using a polynomial of 5th degree (see \ref{section-polynomials-5}). To generate this trajectory, at least 2 points are needed in the cartesian 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3e_joint_polynomial.png}
\caption{Simulation 3e: Polynomial trajectories of 5th degree for each of the 7 joints.% The trajectory is computed at 60 sampling points.
}
...
The goal of this simulation is to generate a trajectory in joint space using a trapezoidal velocity profile. The start and end points as well as the inverse kinematics solutions for these points are the same as those in \ref{section:9-2-polynomial-traj}.

\subsection{Trajectories in joint space with s-curve velocity profile}

The goal of this simulation is to generate a trajectory in joint space using a smooth s-curve velocity profile. The start and end points as well as the inverse kinematics solutions for these points are the same as those in \ref{section:9-2-polynomial-traj}.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3f_trapezoid1.png}
\caption{Simulation 3f: Trajectory using a trapezoid velocity profile ($τ = 15$ seconds). %The trajectory is computed at 60 sampling points.
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{images/robot_planner3/3g_s_curve.png}
\caption{Simulation 3g: Trajectory using a s-curve velocity profile ($τ_1 = τ_2 = 6$) %and the total trajectory duration is 60 seconds. %The trajectory is computed at 60 sampling points.
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner3/3h_helix.png}
\caption{Simulation 3h: Helical trajectory creation inside the surgical site (below the green mounting dock) and subsequent transformation via the fulcrum transformation to a trajectory for the robot's TCP.}
\label{robot-planner3h-helix}
\end{figure}
\end{center}

\section{Robot Planner 4: Simple cube pick-and-place simulation}
%
In this simulation we plan a simple pick-and-place path for a cube. The robotic arm first visits the left table and starts from the pre-grasp posture and then 
...
show some frames from the simulation with the first three to show the pick steps in the simulation environment and then other three images show the place steps 
in the visualization program.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner4/robot_planner4}
\caption{Simulation 4: Simple pick-and-place simulation of a cube}
\end{figure}
\end{center}

Observing the results from table \ref{robot-planner4-rrtconnect-data}, the time duration for the pick pipeline of this simulation using the RRTConnect path planning algorithm, 
...
It is also possible, that due to the obstacle and the initial conditions, the robot may not find a solution.

% Robot Planner 4 with RRTConnect
\begin{longtable}{|p{2cm}|c|p{3cm}|p{3cm}|p{3cm}|}
\hline
Planner 4           & \multicolumn{4}{c}{\textbf{RRTConnect}}                                                                                                 \vline \\
\hline
...
\hline
\caption{Time results for RP 4 using the RRT* path planner algorithm}
...
\begin{longtable}{|p{2cm}|c|p{3cm}|p{3cm}|p{3cm}|}
\hline
Planner 4           & \multicolumn{4}{c}{\textbf{PRM*}}                                                                                                 \vline \\
\hline
                          & \multicolumn{4}{c}{\textbf{Pick Pipeline}}                     \vline \\
\hline
Simulation                & Status & Solution Time & Path Simplification Time & Planning Attempts  \\
...
\hline
\caption{Time results for RP 4 using the PRM* path planner algorithm}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.35\textwidth]{images/robot_planner5/visual_servo_controller3.png}
\includegraphics[width=0.35\textwidth]{images/robot_planner5/visual_servo_controller4.png}\\
\caption{Visual servo controller error diagrams. On the left image in the error graphs appear some spikes. These spikes occur from the sudden temporary detection of a nearby surgical tool. On the right image, these spikes are filtered out, and only the error graphs of the visual servoing of one tool are shown. The  controller parameters are $K_p = 0.9, K_d = 0.2$}
\end{figure}
\end{center}
...
The goal of this simulation is to measure the alignment error of the long axis of the surgical tool with the fulcrum point. This error tracks whether the robot pose satisfies the RCM constraint, 
in which the axis of the tool must always pass through the fulcrum point and is explained in more detail in \ref{section:rcm-tracking}. To simplify the Simulation, 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner6/robot_planner6.png}
\caption{Robot planner 6: Path from home, to point above Fulcrum point and path of insertion/retraction}
\end{figure}
\end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner6/rcm_fulcrum_alignment_error.png}
\includegraphics[width=0.7\textwidth]{images/robot_planner6/rcm_fulcrum_alignment_error2.png}\\
\caption{RCM alignment error, diagrams from 2 executions of the same simulation. The x-axis is measured in seconds (ROS time) and the y-axis is the distance error measured in meters in logarithmic scale.}
\end{figure}
\end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/robot_planner6/topics-and-nodes.png}
\caption{ROS nodes and topics used for the robot-planner6 simulation. In this simulation the trajectories and RCM fulcrum errors are with respect to the 2nd fulcrum reference frame only.}
\end{figure}
\end{center}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.35\textwidth]{images/robot_planner6/rcm-error-collision.png}
\includegraphics[width=0.35\textwidth]{images/robot_planner6/rcm-collision.png}\\
\caption{Surgical tool slides from an aligned, RCM pose to a pose misaligned from the fulcrum point, no RCM motion and surgical tool is exerting pressure to the abdominal wall. The diagram on the 
...
The goal of this simulation is to run all stages of this thesis together to study the end-to-end result. Each stage is run by a different node and the sequence in which every task is executed is dictated by the status of the state machine as shown in Figure~\ref{smack-state-machine}.

\begin{center}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{images/state-machine-all-tasks.png}
