\documentclass[a4paper,11pt,twoside]{book}

\usepackage{my-thesis-template}


%----------------------------------------------------------------------------------------
%	METADATA
%----------------------------------------------------------------------------------------
\newcommand{\thesistitle}{Perception, control and path planning of robotic laparoscopic surgical system}
\newcommand{\thesistitlegr}{Αντίληψη, έλεγχος και σχεδιασμός διαδρομής ρομποτικού λαπαροσκοπικού χειρουργικού συστήματος }
\newcommand{\division}{Division}
\newcommand{\fullname}{Alexios Karadimos }
\newcommand{\me}{\fullname of Loukas}
\newcommand{\nomme}{\fullname of Loukas}
\newcommand{\studnum}{1046820}
\newcommand{\monthyear}{February 2022}
\newcommand{\supname}{Evangelos Dermatas}
\newcommand{\suptitle}{Associate Professor}
\newcommand{\cosupname}{Anthony Tzes}
\newcommand{\cosuptitle}{Professor}
\newcommand{\headofdivision}{Nikolaos Koussoulas}
\newcommand{\headofdivisiontitle}{Professor}
\newcommand{\keywords}{Minimal invasive surgery, perception, control, path planning, fulcrum, ROS, MoveIt}
\title{\thesistitle}
\author{\fullname}
\date{2022}

% PDF metadata
\hypersetup
{
    pdfauthor={\fullname},
    pdftitle={\thesistitle},
    pdfsubject={\thesistitle},
    pdfkeywords={\keywords},
    pdfproducer={\fullname},
    pdfcreator={\fullname}
}


\begin{document}

\selectlanguage{greek}
\selectlanguage{english}


%----------------------------------------------------------------------------------------
%	FRONT PAGE & CERTIFICATION
%----------------------------------------------------------------------------------------
\include{titlepage}
%\newpage{\pagestyle{empty}\cleardoublepage}

\include{copyright-en}

\include{certification}

\include{titlepage-gr}
%\newpage{\pagestyle{empty}\cleardoublepage}

\include{copyright-gr}

\include{certification-gr}

%\include{details}
%\newpage{\pagestyle{empty}\cleardoublepage}

\include{abstract}
\newpage{\pagestyle{empty}\cleardoublepage}

\include{acknowledgements}


%----------------------------------------------------------------------------------------
%	TABLE OF CONTENTS
%----------------------------------------------------------------------------------------

\pagestyle{fancy}

\clearpage
\tableofcontents
\renewcommand*\contentsname{Table of Contents}

%----------------------------------------------------------------------------------------
%	MAIN SECTIONS
%----------------------------------------------------------------------------------------

\newpage
\input{chapters/1_introduction}

\newpage
\input{chapters/2_kinematics}

\newpage
\input{chapters/3_grasping}

\newpage
\input{chapters/4_vision}

\newpage
\input{chapters/5_path}

\newpage
\input{chapters/6_trajectory}

\newpage
\input{chapters/7_control}

\newpage
\input{chapters/8_ros_sim}

\newpage
\input{chapters/9_results}

\newpage
\input{chapters/10_conclusions}


%----------------------------------------------------------------------------------------
%	BIBLIOGRAPHY, REFERENCES, TABLES, APPENDIXES
%----------------------------------------------------------------------------------------
\newpage
\appendix
\appendixpage
\input{chapters/appendix}

\newpage
\input{nomenclature}
\printnomenclature

\listoffigures

\addcontentsline{toc}{section}{\listoflistingscaption}
\listoflistings

\listofalgorithms

\listoftables

\nocite{*}
\printbibliography[heading=bibintoc,title={Bibliography}]

%\end{otherlanguage}
\end{document}
